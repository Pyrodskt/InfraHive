...
class ServerAgentsWorker(threading.Thread):
    """
    ...
    def __init__(self, collector):
        """
        Initialise le ServerAgentsWorker.
    ...
    def run(self):
        ...
        """
        logger.info(f"Démarrage de la collecte et du traitement pour la source : {self.collector.collector_name}")
        ...
        try:
            ...
            if not servers_data:
                logger.warning(f"Aucune donnée à traiter pour la source : {self.collector.collector_name}")
                return
            
            logger.info(f"Collecte terminée : {len(servers_data)} serveurs trouvés pour la source : {self.collector.collector_name}")
            
            # Appel de la nouvelle méthode d'exportation
            self.export_to_json(servers_data, self.collector.collector_name)
            
            # Stockage du résultat pour que le thread principal puisse le récupérer (facultatif, mais garde la compatibilité)
            self.result = {self.collector.collector_name: servers_data}
            
        except Exception as e:
            logger.error(f"Erreur dans le thread du collecteur {self.collector.collector_name} : {e}", exc_info=True)